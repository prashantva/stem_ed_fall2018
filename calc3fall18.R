...
# Linear modeling
fit<-lm(Calc3~Calc2+Calc1+iphone, df);summary(fit); # The linear model
# Now lets do stepwise forward linear model 
fit_backward <- stepAIC(fit, direction="backward", trace=0)

# Now the forward linear model
fit0<-lm(Calc3~ 1, df);# summary(fit0); # The linear model with only intercept
...
Calc3_fit<-fitted(fit_final) # predicted values

# Regression diagnostics 
ggplot(data=df)+
  my_theme+
  coord_fixed()+
  labs(x="Calculus II grade points", y="Calculus III grade points")+
  geom_point(mapping = 
               aes(x = Calc2, y = Calc3, color=Calc1), size=5)+
  geom_line(aes(x=Calc2, y = Calc3_fit), color="black", size=1)
