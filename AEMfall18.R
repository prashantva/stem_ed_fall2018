...
# If we remove the outliers from NMjr, PhOnce, and PhAlways we get a collinearity
# So we remove them
#df<-within(df, rm('GpaPr', 'NMjr', 'PhOnce', 'PhAlwys')); # Remove the GpaPR manually

df<-within(df, rm('GpaPr'));
           
# Linear modeling
fit<-lm(AEM~., df);summary(fit); # The linear model
fit_backward <- stepAIC(fit, direction="backward", trace=0)
fit0<-lm(AEM~ 1, df);# summary(fit0); # The linear model with only intercept
...
AEM_fit<-fitted(fit_final) # predicted values

# Regression diagnostics 
ggplot(data=df)+
  my_theme+
  #coord_fixed()+
  labs(x="Calculus III grade points", y="AEM grade points")+
  geom_point(mapping = 
               aes(x = Calc3, y = AEM, color=Calc2), size=5)#+
  #geom_line(aes(x=Calc3, y = Diffeq_fit), color="black", size=1)#+
...
ggplot(data=df)+
  ...
  labs(x="Calculus III grade points", y="AEM grade points")+
  geom_point(mapping = 
               aes(x = Calc3, y = AEM, color=Diffeq), size=5)
x <- df$Calc3
y <- df$NMnr
x3 <-df$Diffeq
z <- df$AEM
...
scatter3D(x, y, z, pch = 20, cex = 2, 
          theta = -50, phi = 10, ticktype = "detailed",
          xlab = "Calculus 3 grades", ylab = "Number of minors", zlab = "AEM grades",  
...
p<-pwr.f2.test(u = num_DF, v=den_DF, f2=f2medium, sig.level = 0.05, power = NULL)
cat(sep="\n")
cat("Power of the test with medium effect size:", p$power,".")
cat(sep="\n")

# Number of subjects required for 80% power using 1 variable
p<-pwr.f2.test(u = num_DF, v=NULL, f2=f2medium, sig.level = 0.05, power = 0.8)
N80<-ceil(p$u+p$v+1) # 
...
p<-pwr.f2.test(u = num_DF, v = den_DF, f2=f2small, sig.level = 0.05, power = NULL); p$power
cat("Power of the test with small effect size is ", p$power)
cat(sep="\n")
# Number of subjects required for 80% power using 1 variable
p<-pwr.f2.test(u = num_DF, v=NULL, f2=f2small, sig.level = 0.05, power = 0.8)
N80<-ceil(p$u+p$v+1) # 
...
only_screentime<-lm(AEM~Screen+Calc1+Calc2+Calc3+Diffeq+MWF+GradYr+iphone, df)
print(summary(only_screentime))
print(summary(fit))
